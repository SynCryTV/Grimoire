...
-- Einstellungen sind absichtlich NICHT Teil des oberen Button-Stapels.
-- Der Button sitzt separat rechts unten am Grimoire-Fenster.
local SETTINGS_DEF = {
    key = "settings",
    label = "Einstellungen",
...
local function CreateTabButton(index, def, anchorBottom)
    ...
    if anchorBottom then
        btn:SetPoint("BOTTOMLEFT", G.panel, "BOTTOMRIGHT", 4, 20)
    else
        btn:SetPoint(
            "TOPLEFT",
            G.panel,
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText(G.L(def.label))
        GameTooltip:Show()
    end)
...
for index, def in ipairs(TAB_DEFS) do
    tabButtons[def.key] = CreateTabButton(index, def, false)
end

-- Separater Einstellungsbutton unten rechts.
tabButtons[SETTINGS_DEF.key] = CreateTabButton(0, SETTINGS_DEF, true)
