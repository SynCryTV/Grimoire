-- Manuell aktualisiert via classcodex-scraper (nicht offiziell)
ClassCodexArchonStats = ClassCodexArchonStats or {}
ClassCodexArchonStats["ROGUE"] = {
  ["assassination"] = {
    ["Mythic+"] = { targets = { crit = 1131, haste = 747, mastery = 601, versatility = 98 } },
    ["Raid"] = { targets = { crit = 1130, haste = 820, mastery = 498, versatility = 90 } },
  },
  ["outlaw"] = {
    ["Mythic+"] = { targets = { crit = 1359, haste = 927, mastery = 344, versatility = 158 } },
    ["Raid"] = { targets = { crit = 1160, haste = 911, mastery = 295, versatility = 204 } },
  },
  ["subtlety"] = {
    ["Mythic+"] = { targets = { mastery = 1283, haste = 650, crit = 647, versatility = 58 } },
    ["Raid"] = { targets = { mastery = 1043, haste = 905, crit = 571, versatility = 80 } },
  },
}